...
@router.post("/upload-profile-picture", response_model=UserResponse, tags=["User Management Requires (Admin or Manager Roles)"], name="upload_profile_picture")
async def upload_profile_picture(
    ...
    if file.content_type not in ["image/jpeg", "image/png", "image/gif"]:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid file format. Only JPEG, PNG, and GIF are allowed.")
    
    if file.size == 0:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="File is empty.")
    
    max_file_size = 5 * 1024 * 1024  # 5MB limit for example
    if file.size > max_file_size:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="File is too large.")
    
    try:
        file_service = FileService()
        file_url = await file_service.upload_File(file, file.filename)
        updated_user = await UserService.update_profile_picture(db, user_id, file_url)
        return UserResponse.model_construct(
            id=updated_user.id,
            bio=updated_user.bio,
            first_name=updated_user.first_name,
